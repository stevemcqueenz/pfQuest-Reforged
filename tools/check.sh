...
echo "== [1/5] Lua 5.1 parse (the 3.3.5a client's Lua) =="
...
echo
echo "== [2/5] 3.3.5a API surface (vs milkyway-codex) =="
python3 tools/apicheck335.py || fail=1

echo
echo "== [3/5] runtime: build our objects and drive them =="
lua5.1 tools/runtimecheck335.lua || fail=1

echo
echo "== [4/5] end-to-end: load the real tracker and build rows =="
lua5.1 tools/trackercheck335.lua || fail=1

echo
echo "== [5/5] tracking data: db/tracking335.lua and the merge that consumes it =="
