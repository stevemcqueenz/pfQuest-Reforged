...
echo "== [1/8] Lua 5.1 parse (the 3.3.5a client's Lua) =="
...
echo
echo "== [2/8] 3.3.5a API surface (vs milkyway-codex) =="
python3 tools/apicheck335.py || fail=1

echo
echo "== [3/8] runtime: build our objects and drive them =="
lua5.1 tools/runtimecheck335.lua || fail=1

echo
echo "== [4/8] end-to-end: load the real tracker and build rows =="
lua5.1 tools/trackercheck335.lua || fail=1

echo
echo "== [5/8] tracking data: db/tracking335.lua and the merge that consumes it =="
lua5.1 tools/trackingcheck335.lua || fail=1

echo
echo "== [6/8] compass: contract harness =="
lua5.1 tools/compasscheck335.lua compass.lua || fail=1

echo
echo "== [7/8] pins: contract harness =="
lua5.1 tools/pinscheck335.lua pins.lua || fail=1

echo
echo "== [8/8] nameplates: contract harness =="
