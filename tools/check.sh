...
echo "== [1/12] Lua 5.1 parse (the 3.3.5a client's Lua) =="
...
echo
echo "== [2/12] XML well-formedness (the client's parser is not forgiving) =="
...
echo
echo "== [3/12] 3.3.5a API surface (vs milkyway-codex) =="
python3 tools/apicheck335.py || fail=1

echo
echo "== [4/12] runtime: build our objects and drive them =="
lua5.1 tools/runtimecheck335.lua || fail=1

echo
echo "== [5/12] end-to-end: load the real tracker and build rows =="
lua5.1 tools/trackercheck335.lua || fail=1

echo
echo "== [6/12] tracking data: db/tracking335.lua and the merge that consumes it =="
lua5.1 tools/trackingcheck335.lua || fail=1

echo
echo "== [7/12] quest reworks: db/wotlkrework335.lua and the merge that applies it =="
lua5.1 tools/reworkcheck335.lua || fail=1

echo
echo "== [8/12] skill ranks: db/quests-skillrank335.lua, its merge and the filter =="
lua5.1 tools/skillrankcheck335.lua || fail=1

echo
echo "== [9/12] settings window: load config.lua and BUILD it =="
lua5.1 tools/configcheck335.lua || fail=1

echo
echo "== [10/12] compass: contract harness =="
lua5.1 tools/compasscheck335.lua compass.lua || fail=1

echo
echo "== [11/12] pins: contract harness =="
lua5.1 tools/pinscheck335.lua pins.lua || fail=1

echo
echo "== [12/12] nameplates: contract harness =="
