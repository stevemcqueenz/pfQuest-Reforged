...
# World
World<->screen bridge (worldapi flavor). Wraps the client's own projection --
nothing is computed addon-side.

## UnitPosition`API`
Arguments: **unitId** `string`

Returns: **x**`number`, **y**`number`, **z**`number` (world yards) -- nothing when the token does not resolve
...
## WorldToScreen`API`
Arguments: **x**`number`, **y**`number`, **z**`number` (world position)

Returns: **screenX**`number`, **screenY**`number`, **visible**`1nil`

Screen coordinates are in the client's UI coordinate space, origin bottom-left.
`visible` is 1 when the point projects on screen and nil otherwise (behind the
camera / outside the frustum); coordinates are still returned so a marker can be
clamped to the screen edge as a turn hint.
