...
enum class ResourceType : uint32_t {
	Texture, RenderTarget, ShaderVertex,
	ShaderPixel
};
...
void RegisterForCleanup(IUnknown** ppRes, ResourceType type, const ResourceParams& p) {
	...
	g_managedResources.push_back({.ppResource = ppRes, .type = type, .params = p});
}
...
void RegisterPresentCallback(const PresentCallback& callback) { if (callback) g_presentCallbacks.push_back(callback); }

void RegisterBeginSceneCallback(const BeginSceneCallback& callback) { if (callback) g_beginSceneCallbacks.push_back(callback); }

void RegisterEndSceneCallback(const EndSceneCallback& callback) { if (callback) g_endSceneCallbacks.push_back(callback); }

void RegisterDrawPrimitiveCallback(const DrawPrimitiveCallback& callback) { if (callback) g_drawPrimitiveCallbacks.push_back(callback); }

void RegisterDrawIndexedPrimitiveCallback(const DrawIndexedPrimitiveCallback& callback) { if (callback) g_drawIndexedPrimitiveCallbacks.push_back(callback); }

void RegisterSetTextureCallback(const SetTextureCallback& callback) { if (callback) g_setTextureCallbacks.push_back(callback); }

void RegisterSetRenderStateCallback(const SetRenderStateCallback& callback) { if (callback) g_setRenderStateCallbacks.push_back(callback); }

void RegisterSetVertexShaderCallback(const SetVertexShaderCallback& callback) { if (callback) g_setVertexShaderCallbacks.push_back(callback); }

void RegisterSetPixelShaderCallback(const SetPixelShaderCallback& callback) { if (callback) g_setPixelShaderCallbacks.push_back(callback); }

void RegisterCreateTextureCallback(const CreateTextureCallback& callback) { if (callback) g_createTextureCallbacks.push_back(callback); }

void RegisterSetRenderTargetCallback(const SetRenderTargetCallback& callback) { if (callback) g_setRenderTargetCallbacks.push_back(callback); }

void RegisterClearCallback(const ClearCallback& callback) { if (callback) g_clearCallbacks.push_back(callback); }

void RegisterResetCallback(const ResetCallback& callback) { if (callback) g_resetCallbacks.push_back(callback); }


void RegisterOnCreate(const ResourceCallback& callback) { if (callback) g_onCreateCallbacks.push_back(callback); }

void RegisterOnDestroy(const ResourceCallback& callback) { if (callback) g_onDestroyCallbacks.push_back(callback); }

void RegisterOnRelease(const ResourceCallback& callback) { if (callback) g_onReleaseCallbacks.push_back(callback); }

void RegisterOnRestore(const ResourceCallback& callback) { if (callback) g_onRestoreCallbacks.push_back(callback); }


void RegisterVertexShaderInit(const VertexShaderInitCallback& callback) { if (callback) g_vertexShaderCallbacks.push_back(callback); }

void RegisterPixelShaderInit(const PixelShaderInitCallback& callback) { if (callback) g_pixelShaderCallbacks.push_back(callback); }
...
IDirect3DDevice9* GetDevice() {
	__try {
		...
		if (device && device->TestCooperativeLevel() == D3D_OK) { return device; }
		return nullptr;
	}
...
IDirect3DVertexShader9* CompileVertexShader(const ResourceParams& p) {
	...
	HRESULT hr = D3DCompile(p.shaderCode.data(), p.shaderCode.size(), nullptr, nullptr, nullptr, p.entryPoint.c_str(), p.target.c_str(), 0, 0, &pCode, &pError);
	if (FAILED(hr)) {
		LogShaderError(pError, 1);
		return nullptr;
	}
...
bool CreateTexture(IDirect3DTexture9** ppTexture, ResourceParams p) {
	...
	*ppTexture = nullptr;
	if (p.ppSurface) *p.ppSurface = nullptr;
	IDirect3DDevice9* device = GetDevice();
