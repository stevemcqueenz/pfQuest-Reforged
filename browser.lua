...
local function ResultButtonEnter()
  -- Reforged: accent-tinted hover instead of the flat white wash
  local a = pfQuestTheme.accent
  this.tex:SetTexture(a[1], a[2], a[3], 0.09)
...
-- Reforged: the browser is resizable. The four tab lists stretch with the
-- window; only the scroll children carry a fixed width, so OnSizeChanged keeps
-- them (and therefore every result row) in sync. The size persists per char.
pfBrowser:SetResizable(true)
if pfBrowser.SetMinResize then pfBrowser:SetMinResize(640, 420) end
if pfBrowser.SetMaxResize then pfBrowser:SetMaxResize(1280, 960) end

pfBrowser:SetScript("OnSizeChanged", function()
  local w = this:GetWidth()
  if this.tabs then
    for _, tab in pairs(this.tabs) do
      if tab.list then
        tab.list:SetWidth(w - 40)
      end
    end
  end
end)

pfBrowser:SetScript("OnShow", function()
  local size = pfQuest_config and pfQuest_config.browsersize
...
pfBrowser.sizer = CreateFrame("Button", nil, pfBrowser)
pfBrowser.sizer:SetPoint("BOTTOMRIGHT", -1, 1)
pfBrowser.sizer:SetWidth(14)
pfBrowser.sizer:SetHeight(14)
pfBrowser.sizer.tex = pfBrowser.sizer:CreateTexture(nil, "OVERLAY")
pfBrowser.sizer.tex:SetTexture("Interface\\ChatFrame\\UI-ChatIM-SizeGrabber-Up")
pfBrowser.sizer.tex:SetAllPoints()
pfBrowser.sizer:SetScript("OnMouseDown", function()
  pfBrowser:StartSizing("BOTTOMRIGHT")
end)
pfBrowser.sizer:SetScript("OnMouseUp", function()
  pfBrowser:StopMovingOrSizing()
  pfQuest_config.browsersize = { pfBrowser:GetWidth(), pfBrowser:GetHeight() }
end)

-- Reforged: flat dark panel chrome + header strip (replaces the pfUI backdrop)
pfQuestTheme.SkinPanel(pfBrowser, 0.93)
pfQuestTheme.HeaderStrip(pfBrowser, 26)
...
CreateBrowseWindow("items", "pfQuestBrowserItems", pfBrowser, "BOTTOMRIGHT", -179, 5)
CreateBrowseWindow("quests", "pfQuestBrowserQuests", pfBrowser, "BOTTOMRIGHT", -20, 5)
